...
export const imageList: ToolDefinition<ListArgs, ListOutput> = {
  // Named for the question, not for one medium: "find the thing I made
  // earlier" is the same question for a picture and for a video, and
  // the gallery behind it holds both. It was `image_list` until video
  // existed — and then returned videos, which a tool with that name has
  // no business doing.
  name: 'media_list',
  // Its own toolset rather than `image`: a video-only install needs to
  // list what it made, and gating this on imageGen would have left it
  // with no way to.
  toolset: 'media',
  description:
    'Find images and video generated earlier, newest first. Use this to recover the path of ' +
    'something whose tool result has scrolled out of the conversation — "the koala one", "the ' +
    'last video I made". Filter by type (image | video), prompt substring (query), model, ' +
    'agent, or date range. Returns paths and metadata; pass a path to file_read or ' +
    'analyze_file to actually look at it — for a video, analyze_file reads the thumbnail ' +
    'beside it.',
  inputSchema: ListInput,
  jsonSchema: {
    type: 'object',
    properties: {
      type: { type: 'string', description: 'Restrict to "image" or "video". Omit for both.' },
  ...
  // Either surface: the gallery is shared, so listing must work
  // wherever there is something to list.
  available: (ctx) =>
    imageGenEnabled(ctx) ||
    (ctx.config.videoGen?.enabled === true && (ctx.config.videoGen?.models.length ?? 0) > 0),
