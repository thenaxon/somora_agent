...
export function buildSelfPointer(persona: Persona, config: Config, somoraHome: string): string {
  ...
  return [
    ...
    `- Show a local image inline with the same bare absolute path in image syntax: \`![label](/home/.../picture.png)\`. Works for images generated by you or by a subagent (use the path from its result).`,
  ].join('\n');
}
