...
- **A self-hosted build can be a community runtime.** The DeepSeek V4.1
  row above was measured on a community SM120 image with NVMe offload,
  not on stock SGLang. The configuration is what was tested there;
  treat the numbers as that setup's behaviour, not as a property of
